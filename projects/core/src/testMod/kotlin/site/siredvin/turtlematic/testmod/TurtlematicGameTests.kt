...
class TurtlematicGameTests {
    ...
    @GameTest(template = "empty")
    fun readsLegacySoulProgress(helper: GameTestHelper) = helper.immediate {
        val consumedData = CompoundTag().apply {
            put("minecraft:enderman", CompoundTag())
        }
        val customData = CompoundTag().apply {
            put(SoulHarvestRecipeRegistry.CONSUMER_ENTITY_COMPOUND, consumedData)
        }
        val stack = Items.AUTOMATA_CORE.get().defaultInstance
        stack.set(DataComponents.CUSTOM_DATA, CustomData.of(customData))

        val feedable = stack.item as ISoulFeedableItem
        check(feedable.getActiveRecipe(stack) != null) { "Expected legacy soul progress to remain readable" }
    }
    ...
    fun soulVialFillsFromSoulSand(helper: GameTestHelper) = helper.immediate {
        ...
        repeat(3) { index ->
            ...
            check(vial.get(DataComponents.CUSTOM_MODEL_DATA)?.value == index + 1) { "Expected vial fill level ${index + 1}" }
    ...
    fun chunkManagerRoundTripsSavedRecords(helper: GameTestHelper) = helper.immediate {
        ...
        val loaded = ChunkManager.load(input, helper.level.registryAccess())
        check(loaded.hasForceChunk(owner)) { "Expected loaded owner record" }
        check(loaded.save(CompoundTag(), helper.level.registryAccess()) == input) { "Expected chunk record to survive save/load" }
    }

...
open class BaseAutomataCore(
    ...
    constructor(coreTier: IAutomataCoreTier, enableSup: Supplier<Boolean>, turtleIDSup: TurtleIDBuildFunction = TurtleIDBuildFunction.WITHOUT_CORE): this(
        coreTier, Properties().stacksTo(1), enableSup, turtleIDSup
    )
    ...
    override fun appendHoverText(
        ...
        if (InputConstants.isKeyDown(Minecraft.getInstance().window.window, InputConstants.KEY_LSHIFT)) {
            list.add(text(TurtlematicCore.MOD_ID, "core_configuration"))
            list.add(text(TurtlematicCore.MOD_ID, "interaction_radius", coreTier.interactionRadius))
            list.add(text(TurtlematicCore.MOD_ID, "max_fuel_consumption_rate", coreTier.maxFuelConsumptionRate))
            if (coreTier.cooldownReduceFactor != 1.0)
                list.add(text(TurtlematicCore.MOD_ID, "cooldown_reduce_factor", coreTier.cooldownReduceFactor))
            if (coreTier.traits.isNotEmpty())
                TooltipHandlerCollection.getProvidersForItem(this)?.forEach{ it.addTooltip(coreTier, list) }
        } else {
            list.add(text(TurtlematicCore.MOD_ID, "press_for_description"))
        }
    }
}